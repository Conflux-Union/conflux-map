...
public final class MapSyncProtocol {
    ...
    private record LegacyOffer(
        ...
        boolean supportsServerViewDistance
    ) {
    }
    ...
    public static ServerHandshake acceptClient(
        ...
        if (legacy.negotiationSupported()) {
            ...
            final NegotiatedMapSync session = NegotiatedMapSync.server(
                ...
                legacyCapabilities(legacy.supportsServerViewDistance())
            );
            return new ServerHandshake(
    ...
    private static LegacyOffer parseLegacyOffer(final String field) {
        if (field == null) {
            return new LegacyOffer("", false, false, false, false);
        }
        final int marker = field.indexOf("|sync:");
        if (marker < 0) {
            return new LegacyOffer(field, false, false, false, false);
        }
        ...
        boolean serverViewDistance = false;
        for (final String token : field.substring(marker + 1).split("\\|")) {
            if (token.equals("sync:" + MapSyncCompatibility.NEGOTIATION_VERSION)) {
                ...
                sourceLight = true;
            } else if (token.equals("server-view:1")) {
                serverViewDistance = true;
            }
        }
        return new LegacyOffer(
            ...
            sync && serverViewDistance
        );
    }

    private static Map<MapSyncCapability, Integer> legacyCapabilities(
        final boolean serverViewDistance
    ) {
        final EnumMap<MapSyncCapability, Integer> capabilities =
            new EnumMap<>(MapSyncCapability.all());
        if (!serverViewDistance) {
            capabilities.remove(MapSyncCapability.SERVER_VIEW_DISTANCE);
        }
        // Predictor tokens are frozen at the shapes released peers already speak, so any
