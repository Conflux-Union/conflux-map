...
Shared waypoints require the server companion and are disabled by default. A level-2 operator can use `/confluxmap waypoints enable` to enable them, with `disable` and `status` providing the corresponding operations. Once enabled, every player can publish a waypoint visible to everyone in the "Shared Waypoints" list. Publishers can delete their own unmarked waypoints; marking, unmarking, and deleting any waypoint are administrator operations, and marked waypoints move to the "Server Markers" list. Per-world and per-player limits are configurable in `config/confluxmap/server.json`.
