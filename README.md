...
- **Map layers:** The fullscreen map provides Map, Biome, and Load Level modes. Load Level mode offers Status Bands and Exact Levels. The Overworld distinguishes surface and cave layers, the Nether provides current-level and bedrock-roof layers, and the End uses a void-adapted background. Map lighting follows the live vanilla gamma option as well as time of day and nearby block light.
