...
**Waterlogged collapse:** before any opacity or color decision is made about
a block, if that block state carries a non-empty fluid state, it is replaced
with a plain "just the fluid" representation for every purpose downstream
(opacity test, color, tint, everything). This happens on the very first block
fetched and again on every step of the descent below. Practically: a
waterlogged stair, slab, sign, or plant renders as plain water; the solid
part of a waterlogged block is invisible to this pipeline.
...
- Similarly, the first block (not water/ice/air/bubble-column) that differs
  ...
  This is how things like kelp, seagrass, or coral end up rendered as an
  overlay on top of the seafloor.
