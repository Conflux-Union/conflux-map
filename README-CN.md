...
较大的功能模块基于行为规格（见 [`docs/reference-specs/`](docs/reference-specs/README.md)）
从零实现，不依赖任何现有模组的源码。涉及的第三方组件与归属说明见
...
里程碑 M2 为全屏地图新增了基于种子预测的底图和可选的服务端校正同步，依托内置的
原生 [cubiomes](https://github.com/Cubitect/cubiomes) 库
（`cn.net.rms.confluxmap.nativepredict`）与纯逻辑预测代码
（`cn.net.rms.confluxmap.core.predict`）：

- **种子预测底图** —— 把地图拖动到未探索的主世界或末地区域时，会立即显示基于种子
  推算的生物群系、地形高度和 cubiomes 1.17 树木候选；不支持的生物群系会使用确定性
  树冠回退，真实已探索的瓦片会在其上层正常覆盖。
- **结构候选标记** —— 村庄、海底神殿、林地府邸、前哨站、废弃传送门、末地城会以
  半透明候选标记显示，直到真实数据确认或排除。可在设置中开关。
- **预测显示模式** —— 按 `P` 在三种模式间切换：全部（整张地图显示底图，默认）、
  仅已生成（只在服务端已生成的区块上显示底图，可离线使用已持久化的校正）、
  仅已访问（关闭底图，纯 M1 体验）。
- **可选服务端同步** —— 同一个 jar 可作为服务端 companion 运行，将预测与真实世界
  对比后返回紧凑的按列校正；只有在服务端显式开启时才会共享种子。校正瓦片与已生成
  标记按世界和维度持久化，重连后保留。
