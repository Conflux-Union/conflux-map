...
//#endif
    ...
    /**
     * Both tests write a stone floor into the tile they derive from their own test origin, and
     * vanilla runs a batch's tests concurrently a few dozen blocks apart - so both origins land in
     * one 256-block tile and both floors target the same 64x64 patch of it. Each then picks its
     * floor Y independently from its own area's tallest column, overwrites the other's pixels at
     * the taller Y, and whichever test reads the shared tile sees a neighbor at the wrong height.
     * The bombardment run has to stay beside its own structure for the TNT to tick, so the round
     * trip is the one that steps away - far enough that no batch layout can ever reach it.
     *
     * <p>The offset also has to clear {@code PredictionQualityCorpus}'s +-64 tile radius. On
     * 1.17.1 the batch sits at a hardcoded {@code (0, 4, 0)}, so the round trip's tile is just
     * this offset, and anything smaller would let it build a stone floor on terrain the quality
     * test reads. The bombardment tile stays at 0, which the corpus already excludes.
     */
    private static final int ROUND_TRIP_TILE_OFFSET = 128;
    ...
    public void stoneFloorRoundTripsFromServerWorldToClientPrediction(final TestContext context) {
        ...
        final int tileX = Math.floorDiv(testOrigin.getX(), 256) + ROUND_TRIP_TILE_OFFSET;
        final int tileZ = Math.floorDiv(testOrigin.getZ(), 256) + ROUND_TRIP_TILE_OFFSET;
            ...
            for (int z = 0; z < FLOOR_SIZE; z++) {
                for (int x = 0; x < FLOOR_SIZE; x++) {
                    ...
                    if (x > 0 && x < FLOOR_SIZE - 1 && z > 0 && z < FLOOR_SIZE - 1
                        ...
                        // "absent" means the server dropped it, a mismatched y means something
                        // else wrote into this tile.
